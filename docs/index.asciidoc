...
https://github.com/awslabs/amazon-kinesis-client[configuration objects]
(`CheckpointConfig`, `CoordinatorConfig`, `LeaseManagementConfig`,
`LifecycleConfig`, `MetricsConfig`, `ProcessorConfig`, `RetrievalConfig`, and
`PollingConfig` — the latter covering settings such as `max_records`,
`retry_get_records_in_seconds`, and `max_get_records_thread_pool`).
Each `snake_case` key you provide is matched against the setter methods exposed
by those objects and applied to whichever one accepts it. Some setters take
complex types, which are not supported; only setters that take a primitive can
be configured this way.

Example: 

To set the dynamodb read and write capacity values, use the `LeaseManagementConfig`
setters `initialLeaseTableReadCapacity` and `initialLeaseTableWriteCapacity`. 
