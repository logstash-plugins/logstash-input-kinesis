...
objects, and some settings (for example `list_shards_backoff_time_in_millis` and
`max_list_shards_retry_attempts`) exist on more than one object and are read
independently by KCL. Grouping lets you give each object its own value:

[source,text]
----
additional_settings => {
  "lease_management_config" => { "list_shards_backoff_time_in_millis" => 3000 }
  "retrieval_config"        => { "list_shards_backoff_time_in_millis" => 7000 }
}
----

See <<plugins-{type}s-{plugin}-additional_settings>> for the full list of groups.
...
The KCL 2.x configuration is split across several
https://github.com/awslabs/amazon-kinesis-client[configuration objects]. Settings
are provided grouped by the configuration object they apply to, so the same
setting can take different values on different objects. Each group is a hash of
`snake_case` setter => value pairs. Some setters take complex types, which are not
supported; only setters that take a primitive can be configured this way.

The supported groups are: `checkpoint_config`, `coordinator_config`,
`lease_management_config`, `lifecycle_config`, `metrics_config`,
`processor_config`, `retrieval_config`, and `polling_config` (the latter covering
settings such as `max_records`, `retry_get_records_in_seconds`, and
`max_get_records_thread_pool`).
...
----
additional_settings => {
  "lease_management_config" => { "initial_lease_table_read_capacity" => 25 "initial_lease_table_write_capacity" => 100 }
